...
const handler = async (m, { conn, args, usedPrefix }) => {
    ...
    try {
        ...
        if (!text.trim()) {
            await m.reply(`تخيل و انا سأرسل لك ما تخيلته انا ماتريكس استطيع فعلها نعم ! \nمـــثال:\n*.artdraw* girl with hijan  style`);
            return;
        }
        ...
        await conn.sendMessage(m.chat, { image: { url: media }, caption: `instagram.com/matrix__tv5` }, { quoted: m });
    } catch (error) {
        console.error(error);
